...
export default function WebinarPage() {
  ...
  if (!event) {
    return <div className="p-6 text-red-600">Webinar not found</div>;
  }
  ...
  return (
    <div className="p-6">
      ...
      <a href={event.url} target="_blank" rel="noreferrer" className="bg-purple-600 text-white px-4 py-2 rounded">
        Register
      </a>
      {related.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-semibold mb-4">Related Webinars</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {related.map((e) => (
              <a key={e.id} href={`/webinar/${e.slug}`}>
                <div className="border p-3 rounded">{e.title}</div>
              </a>
            ))}
          </div>
        </div>
